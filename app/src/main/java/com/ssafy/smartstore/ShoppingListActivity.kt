...
class ShoppingListActivity : AppCompatActivity(), ShoppingListDeleteClickListener {
    ...
    private fun initViews() = with(binding) {
        ...
        // 주문상세 리사이클러뷰 연결
        adapter = OrderDetailAdapter(windowState.SHOPPINGLIST, this@ShoppingListActivity).apply {
        ...
        btnOrder.setOnClickListener {
            ...
            if (!StoreApplication.shoppingList.isEmpty()) {
                ...
                if (orderTable == "") {
                    ...
                    val orderDetails = mutableListOf<com.ssafy.smartstore.remote.dto.OrderDetail>()
                    data.forEach {
                        val orderDetail =
                            com.ssafy.smartstore.remote.dto.OrderDetail(it.product.id, it.quantity)
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        com.ssafy.smartstore.remote.repository.OrderRepository.INSTANCE.makeOrder(
                            order
                        )
